...
export class Sequence {
  ...
  succeed(successState) {
    ...
    seq.state = successState; // stop matching against any less preferable alternate sequences

    seq = seq.removeWorse(); // Fixup the reference which we bound too early
  ...
  replaceWith(result, context) {
    ...
    if (result.type === 'success') {
      ...
      const expr = global ? new Expression(engine, engine.makeRootState(context), globalIdx + 1, this) : null;
      return this.succeed({
        type,
        expr,
        captures: [captures]
...
export class Expression {
  constructor(engine, result, globalIdx, parentSeq = null) {
    this.engine = engine;
  ...
  step1(chr) {
    ...
    while (seq !== null) {
      ...
      if (state.type !== 'cont') {
        ...
        const {
          next
        } = state; // width must be 1 here
        // it should be as step0 should always be run first

        const result = next.match(matchState, chr, code(chr), noContext);
