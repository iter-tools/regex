 ...
 * @generatedfrom ./regex.ts#1645382246906
...
const literal = (desc, test, negate = false) => next => {
  const cont = {
    type: 'cont',
    next
  };
...
const edgeAssertion = (kind, flags) => next => {
  ...
  return {
    ...
    width: 0,
    match: flags.multiline ? (state, context) => {
      const {
        atStart,
        atEnd,
        lastCode,
      ...
      return kind === 'start' ? atStart || !testNotNewline(lastCode) ? cont : null : atEnd || !testNotNewline(nextCode) ? cont : null;
    } : (state, context) => {
      const {
        atStart,
        atEnd
...
const repeat = (exp, key, greedy = true) => next => {
  // eslint-disable-next-line prefer-const
  let repeatCont; // eslint-disable-next-line prefer-const
...
const visitExpression = (alternatives, state, visit) => {
  ...
  const reset = resetRepetitionStates(qIdxs, state.initialRepetitionStates); // prettier-ignore
  ...
  switch (alternatives.length) {
    ...
    case 1:
      return compose(reset, visit(alternatives[0]));

    default:
      return expression(alternatives.map(alt => compose(reset, visit(alt))));
  }
};
...
const visitors = {
  ...
  Pattern: (node, state, visit) => {
    ...
    state.initialRepetitionStates[qIdx] = {
      min: 0,
      max: Infinity
    };
    return expression([compose(!state.flags.multiline && isAnchored(node) ? identity : // Allow the expression to seek forwards through the input for a match
  ...
  Quantifier: (node, state, visit) => {
    const {
      ...
      max,
      greedy
    } = node; // See https://github.com/mysticatea/regexpp/issues/21
