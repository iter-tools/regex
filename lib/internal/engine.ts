...
/*
The regex engine data structure:
                                                            ┌───────────────┐
                                                            │  Expression   │
                                                            │  root: true   │
                                                            │ globalIdx: 1  │
                          ...
                          null ◄──┤   Sequence   │◄─────────┤   Sequence    │
                              ...
                              parentExpr │ ▲
                                  ...
                                  │  Expression  │    │   matchers at each step.    │
                                  │              │    │                             │
                                  └──────┬───────┘    │ - A sequence will hold      │
                               parentSeq │ ▲          │   multiple states over time.│
                                         │ │ expr     │                             │
        ┌──────────────┐          ┌──────┴─┴─────┐    │ - Expressions with only one │
        │    State     │          │    State     │    │   sequence are crushed into │
        │  type: cont  │          │  type: expr  │    │   their parent expression.  │
        └──────────────┘          └──────┬───────┘    │                             │
                 ▲                       │ ▲          │ - https://asciiflow.com     │
                 │ state                 ▼ │ state    │                             │
 better ┌────────┴─────┐  better  ┌────────┴─────┐    └─────────────────────────────┘
...
It's not the most efficient structure, no, but at least I'm pretty sure it's correct!
Note the multiple root expressions. Each is the start of a potential match in a global
regex. We don't actually know where in the input to start looking for the next match
until the previous match terminates. This means that matches anchored at different
locations in the input stream may be related in the same tree.

This structure was chosen as even large branches of speculative matching can be trimmed
by adjusting a few references.
*/

export class Sequence {
  // expression state is used to nest expressions in sequences
  state: State;
  ...
  replaceWith(result: Result | null, context: Context): Sequence | null {
...
/**
 * An expression constructs a linked list of sequences and serves as its head
 */
export class Expression {
  ...
  isRoot: boolean;
  globalIdx: number; // Disambiguate between successive occurences of the pattern when matching globally
    ...
    while (seq !== null) {
      ...
      if (state.type !== 'cont') {
        ...
        if (next.width === 0) {
          ...
          seq = seq.replaceWith(result, context);
        } else if (atEnd) {
          seq = seq.fail();
        } else {
          seq = seq.next;
  ...
  step1(chr: string) {
    ...
    while (seq !== null) {
      ...
      if (state.type !== 'cont') {
        ...
        if (result === null) {
          // If we fail, don't do anything else
          seq = seq.fail();
        } else {
