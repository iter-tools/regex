 ...
 * @generatedfrom ./regex.ts#1655756412146
      ...
      match: flags.multiline
        ? kind === 'start'
          ? (state, context) => {
              ...
              return lastCode === -1 || !(0, _literals.testNotNewline)(lastCode) ? next : null;
            }
          : (state, context) => {
              const { nextCode } = context;
              return nextCode === -1 || !(0, _literals.testNotNewline)(nextCode) ? next : null;
            }
        : kind === 'start'
        ? (state, context) => {
            const { lastCode } = context;
