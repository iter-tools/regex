 ...
 * @generatedfrom ./regex.ts#1653251722777
...
const term = (global, capturesLen) => ({
  ...
  match: (state) => {
    ...
    return {
      ...
      global,
      captures: (0, _captures.flattenCapture)(capture, capturesLen),
    };
  }
});
...
const buildPatternInternal = (ast, flags) => {
  ...
  const seq = (0, _ast.visit)(ast, state, visitors);

  const initialState = {
    result: null,
    parentCaptures: _immStack.default,
    capture: null,
  ...
  const matcher = seq(term(state.flags.global, state.cIdx + 1));
