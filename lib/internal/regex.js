 ...
 * @generatedfrom ./regex.ts#1647109390482
    ...
    return {
      ...
      match: flags.multiline
        ? (state, context) => {
            ...
            return kind === 'start'
              ? atStart || !(0, _literals.testNotNewline)(lastCode)
                ? cont
                : null
              : atEnd || !(0, _literals.testNotNewline)(nextCode)
              ? cont
              : null;
          }
        : (state, context) => {
...
const boundaryAssertion = () => (next) => {
  ...
  return {
    ...
    match: (state, context) => {
      const { atStart, atEnd, lastCode, nextCode } = context;
      const lastIsWord = atStart ? false : (0, _literals.testWord)(lastCode);
      const nextIsWord = atEnd ? false : (0, _literals.testWord)(nextCode);
...
const visitors = {
  ...
  Pattern: (node, state, visit) => {
    ...
    return expression([
      compose(!state.flags.multiline && (0, _ast.isAnchored)(node)
        ? identity
  ...
  CharacterSet: (node, state) => {
    const tester = (0, _literals.getTester)(node, state.flags);
    const desc = (0, _ast.getCharSetDesc)(node);
    if (node.kind === 'any') {
