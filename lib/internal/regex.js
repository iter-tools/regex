 ...
 * @generatedfrom ./regex.ts#1653681585102
...
const edgeAssertion =
  (kind, flags) => (next) => {
    const match = flags.multiline
      ? kind === 'start'
        ? function match(state, context) {
            const { lastCode } = context;
            return lastCode === -1 || !(0, _literals.testNotNewline)(lastCode) ? this.next : fail;
          }
        : function match(state, context) {
            const { nextCode } = context;
            return nextCode === -1 || !(0, _literals.testNotNewline)(nextCode) ? this.next : fail;
          }
      : kind === 'start'
      ? function match(state, context) {
          const { lastCode } = context;
