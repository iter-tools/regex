 ...
 * @generatedfrom ./chunked.ts#1649620510882
...
const { exec, test, execGlobal } = new _api.AsyncApi(async function* generate(pattern, iterable) {
  const engine = new _engine.Engine(pattern);
  let chunkPeekr = await (0, _asyncPeekerate.default)(
    (0, _utils.map)(iterable, _peekerate.default),
  );
